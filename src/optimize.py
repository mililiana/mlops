...
def load_processed_data(
    ...
    if abs_path.endswith((".pkl", ".pickle")):
        ...
        if isinstance(obj, dict):
            ...
            if {"X", "y"}.issubset(obj.keys()):
                ...
                raise ValueError(
                    "Unknown format ({X,y} or {X_train,X_test,y_train,y_test})."
                )
    ...
    if abs_path.endswith(".csv"):
        ...
        # Using 'label' as target, and 'tweet' as the text feature
        if "label" not in df.columns or "tweet" not in df.columns:
            raise ValueError("CSV should contains 'label' and 'tweet' columns.")

        vectorizer = TfidfVectorizer(
            max_features=30000, analyzer="char_wb", ngram_range=(2, 6), min_df=2
        )
        X = vectorizer.fit_transform(df["tweet"].fillna("")).toarray()
        y = df["label"].values
        X_train, X_test, y_train, y_test = train_test_split(
            X,
            y,
...
def evaluate(
    ...
    if metric == "f1":
        if hasattr(model, "predict_proba") and len(np.unique(y_test)) == 2:
            ...
            # find best threshold on train set to avoid leaking test set
